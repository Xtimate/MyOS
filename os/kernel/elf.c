...
unsigned int elf_load(void *elf_data, unsigned int pd_phys, unsigned int *brk_out) {
    ...
    for (int i = 0; i < hdr->e_phnum; i++) {
        ...
        unsigned int seg_end = (ph->p_vaddr + ph->p_memsz + 0xFFF) & ~0xFFF;
        if (seg_end > top) top = seg_end;
    }

    if (brk_out) *brk_out = top;
    return hdr->e_entry;
}
